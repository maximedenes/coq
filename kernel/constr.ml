...
type ('constr, 'types, 'sort, 'univs) kind_of_term =
  ...
  | Int       of Uint63.t
  | Float     of Float64.t
  | Array     of 'constr array * 'constr * 'types
...
(* Constructs an array *)
let mkArray (t,def,ty) = Array (t,def,ty)
...
let iter f c = match kind c with
  ...
  | Array(t,def,ty) -> Array.iter f t; f def; f ty
...
let fold_with_full_binders g f n acc c =
  ...
  | CoFix (_,(lna,tl,bl)) ->
      ...
      let fd = Array.map2 (fun t b -> (t,b)) tl bl in
      Array.fold_left (fun acc (t,b) -> f n' (f n acc t) b) acc fd
  | Array(t,def,ty) -> f n (f n (Array.fold_left (f n) acc t) def) ty
...
let compare_head_gen_leq_with kind1 kind2 leq_universes leq_sorts eq leq nargs t1 t2 =
  ...
  | Array(t1,def1,ty1), Array(t2,def2,ty2) ->
     Array.equal_norefl (eq 0) t1 t2 && eq 0 def1 def2 && eq 0 ty1 ty2
...
let hasheq t1 t2 =
  match t1, t2 with
    ...
    | Float f1, Float f2 -> Float64.equal f1 f2
    | Array(t1,def1,ty1), Array(t2,def2,ty2) ->
      def1 == def2 && ty1 == ty2 && array_eqeq t1 t2
  ...
  let rec hash_term t =
    match t with
      ...
      | Array (t,def,ty) ->
        ...
        let h = combine3 ht hdef hty in
        (Array(t,def,ty), combinesmall 20 h)
...
let rec hash t =
  match kind t with
    ...
    | Array(t,def,ty) ->
      combinesmall 20 (combine3 (hash_term_array t) (hash def) (hash ty))
...
let rec debug_print c =
  ...
  | Array(t,def,ty) ->
    str"Array(" ++ prlist_with_sep pr_comma debug_print (Array.to_list t) ++
    str" | " ++ debug_print def ++ str" : " ++ debug_print ty ++ str")"
