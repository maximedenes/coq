...
let extraction_err loc =
  if not (Mltop.module_is_known "extraction_plugin") then
    Errors.user_err_loc (loc,"_",str "Please do first a Require Extraction.")
  else
...
let funind_err loc =
  if not (Mltop.module_is_known "recdef_plugin") then
    Errors.user_err_loc (loc,"_",str "Please do first a Require Import FunInd.")
  else
...
GEXTEND Gram
  ...
  command:
	...
	s = [ s = ne_string -> s | s = IDENT -> s ] ->
      ...
      | IDENT "proof" ->
         Errors.user_err_loc
           (!@loc,"_", str"Please do first a Require DeclarativeMode.")
