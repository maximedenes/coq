...
let change_vars =
  let rec change_vars mapping rt =
    DAst.map_with_loc (fun ?loc -> function
      ...
      | GArray(t,def) ->
        GArray(Array.map (change_vars mapping) t, change_vars mapping def)
      ) rt
...
let rec alpha_rt excluded rt =
  ...
  let new_rt = DAst.make ?loc @@
    ...
    | GArray(t,def) ->
      GArray(Array.map (alpha_rt excluded) t, alpha_rt excluded def)
  in
  new_rt
...
let is_free_in id =
  let rec is_free_in x = DAst.with_loc_val (fun ?loc -> function
    ...
    | GArray(t, def) -> Array.exists is_free_in t || is_free_in def
    ) x
...
let replace_var_by_term x_id term =
  let rec replace_var_by_pattern x = DAst.map (function
      ...
      | GArray(t,def) ->
        GArray(Array.map replace_var_by_pattern t, replace_var_by_pattern def)
    ) x
...
let expand_as =
  ...
  let rec expand_as map = DAst.map (function
      ...
      | GArray(t,def) ->
        GArray(Array.map (expand_as map) t, expand_as map def)
    )
